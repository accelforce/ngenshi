...
const toot = async (domain, accessToken, status, spoiler = '', visibility = 'unlisted') => {
    const url = `https://${domain}/api/v1/statuses`;
...
const genshijin = async (text, domain, accessToken) => {
    text = striptags(text);
    ...
    await toot(domain, accessToken, status);
};
